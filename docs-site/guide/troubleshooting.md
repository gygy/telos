...
- 单实例开关改动需**重启应用**生效；PiDeck 的不同版本可以并行运行（按版本互斥），不要误判为 bug。
...
PiDeck 默认发送**匿名、低频**的 `app_heartbeat`（版本分布/平台/活跃数量），不收集项目路径、代码、消息内容；可在设置 → 开发 → 隐私 → 遥测关闭。诊断报告导出前请确认不含敏感信息（报告本身已脱敏，但仍建议自查）。
