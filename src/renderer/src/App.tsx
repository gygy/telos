...
export function App() {
  ...
  useEffect(() => {
    ...
    void api.app
      ...
      .then((info) => {
        ...
        document.title = info.devBranch ? `Telos · ${info.devBranch}` : "Telos";
      })
      .catch(() => undefined);
  ...
  const sessionTabsBarNode = (
    <SessionTabsBar
      {...sessionTabsProps}
      // 「显示对话」常驻 Tab 栏。只在预览占满、对话被收起时拉回分屏；已是分屏时不切换。
      onShowChat={() => {
        if (workbenchLayout !== "maximize" || !workbenchHasContent) return;
