...
export const zhCN = {
  ...
  "tabs.toolsGroup": "工具",
  "tabs.previewHint": "预览「{name}」：发消息或双击后固定；关闭标签不会删除会话",
  "tabs.placeholderNewSession": "新会话",
  "tabs.placeholderNewSessionHint": "发送第一条消息后才会创建会话",
  ...
  "git.gitNotInstalledDesc": "源代码管理功能需要 Git。请前往 https://git-scm.com 安装 Git 后重启 Telos。",
  "browser.devicePC": "PC",
  ...
  "app.logoLabel": "Telos 标志",
  "app.homepage": "官网主页",
  "about.title": "关于 Telos",
  ...
  "app.modelListEmptyGuide": "没有可用模型。请到「设置 → 配置管理 → 模型」添加 provider 与模型，然后点击刷新。",
  ...
  "app.modelListFailConfigInvalid": "模型配置文件（models.json / auth.json）格式有问题，pi 读不了模型。请到「设置 → 配置管理 → 模型」修复配置（或删除损坏文件重建），再点刷新重试。",
  "app.modelListFailCliFailed": "pi --list-models 执行失败，拿不到模型列表。请点刷新重试；若持续失败，可在设置页检查 pi 健康状态。",
  "app.modelListFailEmpty": "没有可用模型：pi 目录与本地 models.json 均无模型。请到「设置 → 配置管理 → 模型」添加 provider 与模型。",
  ...
  "app.preloadMissing": "桌面端预加载 API 未注入，请重新启动 Telos。",
  ...
  "app.emptyGuide": "从左侧选择一个会话开始对话，或点击「新建会话」。\n左侧可以切换项目与历史会话。",
  ...
  "longTask.running": "正在执行。继续输入会排队，本轮结束后发出。停止后队列仍在，文件可从检查点回退。",
  "longTask.runningQueued": "正在执行，还有 {n} 条排队。停止后这些消息仍在，文件可从检查点回退。",
  "longTask.queued": "还有 {n} 条排队，空闲后会发出。停止不会丢掉这些消息。",
  "longTask.interrupted": "已中断。排队消息仍保留，可以直接继续发送；需要回退文件时打开检查点。",
  "longTask.review": "审阅",
  "longTask.checkpoint": "检查点",
  "review.title": "审阅",
  "review.hint": "本轮改动与命令",
  "review.narrative": "这里集中看这一轮改了哪些文件、跑了哪些命令。时间线仍是完整过程；检查点用来回退文件。",
  "review.files": "文件",
  "review.commands": "命令",
  "review.empty": "这一轮还没有可审阅的改动",
  "review.emptyHint": "Agent 写入文件或执行命令后会出现在这里。",
  "review.noSession": "打开会话后，这里汇总本轮改动",
  "review.noSessionHint": "文件差异、命令摘要和检查点入口会集中在这一页。",
  "review.openCheckpoint": "打开检查点",
  "review.commandFailed": "失败",
  ...
  "session.backendLockedBar": "{backend} 会话，换后端请新建",
  ...
  "config.dsh.pluginClientCodeHint": "Client 半区需 dsh-web 浏览器页面，Telos 桌面端不渲染——请只填写 Host 半区。",
  ...
  "config.dsh.staticPluginsHint": "当前部署 cordis 组合中的 Loader 条目（同一模块的多条 Loader 条目已合并成一行，仅展示一次）。来源栏区分「自带」（dsh base / 随包预设 / Telos 组合）与「用户安装」（$DSH_HOME/cordis.patch.yml 用户补丁层）；用户安装的条目可就地卸载（先备份补丁文件再移除行，Telos 管理目录内的插件文件一并移入回收站），host 重启后生效。",
  ...
  "config.dsh.pluginFilesKeepHint": "已从用户补丁层移除；插件文件不在 Telos 管理目录内，请手动删除",
  ...
  "dsh.runtime.managedHint": "按需下载到应用数据目录，可卸载；升级 DSH 不必重装 Telos。",
  "dsh.runtime.outdatedTitle": "DSH runtime 版本不配套",
  "dsh.runtime.outdatedDesc": "当前安装的 runtime 是 v{installed}，与本版本 Telos 配套的 v{declared} 不一致。为避免协议不兼容导致异常，DSH 已暂停使用，请重新安装配套版本。",
  "dsh.runtime.sendOutdated": "DSH 后端运行时（v{installed}）与本版本 Telos 配套版本（v{declared}）不一致，已停止使用。请到配置管理页重新安装后再发送。",
  ...
  "config.tokendance.keyDesc":
    "模型请求按 TokenDance 账户计费，需要 API Key。可以粘贴已有 Key，或通过授权页自动创建（归因到 Telos）。",
  ...
  "config.tokendance.waitingBrowser": "已在浏览器打开授权页：完成登录后 Telos 会自动接收 API Key 并写入配置，无需手动复制。",
...
"config.emptyExtensions": "暂无已安装扩展。",
  ...
  "config.recommendedPackagesHint": "以下扩展与 Telos 兼容，可直接安装使用。部分 CLI 特有功能（如 TUI 界面）在桌面端可能不适用。",
  ...
  "config.extRecommended.piDeckTodo": "Telos 内置：当前有效计划 TODO 扩展，支持显式替换、恢复和分支级持久化，并在对话中追踪任务进度。",
  "config.extRecommended.piDeckPlanMode": "Telos 内置：计划模式扩展，让 AI 在回复前首先生成执行计划，复杂任务一目了然。",
  "config.extRecommended.piDeckGoalMode": "Telos 内置：目标模式扩展，围绕一条目标自动连续推进，直到完成、阻塞或你暂停。",
  "config.extRecommended.piDeckAskQuestion": "Telos 内置：在对话中插入精心设计的问题卡片，引导 AI 给出更精准的回答。",
  "config.extRecommended.piDeckNulRedirectFix": "Telos 内置：修复 Windows 下 pi 重定向到 NUL 设备时可能产生的残留文件问题。",
...
"config.proxyUrlUnset": "未配置代理 URL",
  ...
  "config.im.guideMethodAStep3": "在 Telos 填入配置，点击保存后连接",
  ...
  "config.im.guideMethodBStep6": "在「凭证与基础信息」中获取 App ID 和 App Secret，填入 Telos",
  ...
  "settings.vision.configFileDesc": "扩展与 Telos 共享这份文件：脱离 Telos 单独使用 pi 时也可手动编辑生效。",
  ...
  "settings.backup.createDesc": "把当前模型配置、pi 设置与 Telos 设置保存为一份快照",
  ...
  "usageStats.notInstalled.backfill": "插件只记录安装之后的对话。如需补上历史用量，可执行 /analytics import 回填，在 pi 终端或 Telos 启动的 Agent 中均可",
  ...
  "settings.electronSandboxDesc": "控制 Electron 渲染进程沙箱（与 pi Agent 无关）。默认关闭以兼容 Windows 安全软件/旧 GPU 驱动；开启后需重启 Telos。若启动异常请改回关闭。",
  ...
  "settings.shellContextMenuSectionDesc": "在 Windows 文件夹右键菜单中提供「用 Telos 打开」入口，对本机当前用户生效（写入用户注册表，便携版同样可用）。",
  "settings.shellContextMenu": "在文件夹右键菜单中打开项目",
  "settings.shellContextMenuDesc": "在文件夹图标（及空白处）右键菜单显示「用 Telos 打开」：已收录的目录直接跳转到对应项目，未收录的目录会先确认添加为项目。",
  ...
  "settings.dshRunnerNodeDesc": "Windows 上 DSH 沙箱需要 Node 24 的 node.exe（控制台程序），用来避免 electron.exe 弹出黑窗口。不必改系统 PATH：本机可以继续用 Node 22/25。Telos 会自动找 nvm / fnm / mise / Scoop 里的 24；没有则从应用更新源（默认 AtomGit）下载一份只给本应用用的副本，不进安装包。",
  ...
  "settings.dshRunnerNodeSourceSidecar": "Telos 专用副本",
  ...
  "settings.dshRunnerNodeCoexistHint": "PATH 上的 node 可以不是 24。nvm / fnm 用户请选 v24.x 目录里的 node.exe，不要选 current 垫片。一键下载走应用更新源（AtomGit/GitHub），只写到 Telos 数据目录。",
...
"settings.piProxyModelsNoModelHint": "去 Pi 管理 → 模型 添加模型后即可配置代理",
  ...
  "settings.expandInterimDuringStreamDesc": "开启时，最新轮流式输出时自动展开思考与工具详情。关闭（默认）时保持折叠，节省渲染资源，对话中间输入与滚动更跟手；手动展开的轮次不受影响。",
  ...
  "settings.updateInstallUnsavedMessage": "重启安装会关闭 Telos。请先保存更改，或确认放弃。",
  ...
  "settings.webQrDesc": "用手机扫描二维码，直接打开 Telos Web 服务",
  ...
  "settings.webQrAlt": "Telos Web 服务二维码",
  ...
  "update.readyToInstall": "Telos v{version} 已下载完成，重启即可安装。",
  "update.installFailedTitle": "更新安装未启动",
  "update.installFailedDetail": "安装未能启动：{error}",
  "update.saveBeforeInstallTitle": "请先保存文件更改",
  "update.saveBeforeInstallDetail": "Telos 未能保存所有打开文件的更改。请处理编辑器中的错误后重试。",
  ...
  "update.availableToast": "Telos v{version} 可用，可在设置中下载。",
  "update.availableManualToast": "Telos v{version} 可用，请从 Release 页面手动下载安装。",
  ...
  "update.dotHintBody": "侧栏设置按钮上的圆点表示有可用更新（Telos 应用 / pi CLI / 模型目录）。",
