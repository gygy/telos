...
function environmentLines(env: HealthReport["environment"]): string[] {
	...
	return [
		`- Telos ${env.appVersion} (${env.installMode})`,
		`- OS: ${osLabel} (${env.arch})`,
...
export function formatMarkdown(report: HealthReport, context: HealthReportContext): string {
	const lines: string[] = [];
	lines.push(`# Telos Diagnostic Report`);
	lines.push("");
...
export function formatCard(report: HealthReport, context: HealthReportContext): string {
	const lines: string[] = [];
	lines.push(`📋 Telos 诊断 | ${report.environment.appVersion}`);
	lines.push("");
	...
	lines.push("");
	lines.push(`_由 Telos 生成 · ${formatTime(report.generatedAt)}_`);
	return lines.join("\n");
}
 ...
 * 直接填进 Telos 新会话的输入框（此时 pi 会自动加载项目 AGENTS.md 与技能）。
 */
export function formatAiPrompt(
	...
	const lines: string[] = [];
	lines.push(
		`你是一名专业的桌面软件技术支持工程师。请根据下面的 Telos 诊断报告，判断可能的问题根因，并给出**分步骤、可执行**的排查和修复建议。`,
	);
	lines.push(
	...
	if (projectContext) {
		...
		lines.push(`项目地址（源码仓库）：https://github.com/gygy/telos`);
		lines.push("");
		lines.push(
			`本次分析基于 Telos 工程。项目根目录的 AGENTS.md 记录了编码规范、架构约束与测试门禁，` +
