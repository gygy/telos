 ...
 * 被唤起实例的 argv 携带 launch URL。支持：
 * - telos://session/<uuid>（当前）/ pideck://session/<uuid>（兼容旧 toast）
 * - telos://agent/<uuid> / pideck://agent/<uuid>
 */
const FOCUS_UUID = "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}";
const SESSION_RE = new RegExp(`(?:telos|pideck)://session/(${FOCUS_UUID})`, "i");
const AGENT_RE = new RegExp(`(?:telos|pideck)://agent/(${FOCUS_UUID})`, "i");
