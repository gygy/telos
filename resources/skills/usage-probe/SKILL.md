---
name: usage-probe
description: 为 Telos 的「用量查询」功能排查/扩展供应商支持。当用户想显示某个供应商的用量、余额或额度点数时，先判断是否已内置支持（内置无需配置）；不在内置时引导用户使用「用量查询」弹窗里的通用模板 / New API 模板；两种模板都覆盖不了的接口，帮用户写出 usage-probes.json 的旧版探针数组。
---
