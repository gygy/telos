---
name: telos-doctor
description: 排查 Telos 桌面应用的问题。当用户说「Telos 出问题了」「帮我诊断 Telos」「会话起不来」「pi 连不上」「应用报错」等时使用。引导用户在「问题反馈」页生成环境诊断报告并导出，读取诊断报告定位根因，结合故障模式库给出可执行的修复步骤。报告含脱敏后的环境信息、体检项、最近报错日志，不接触任何原始配置文件内容。
---

# Telos 环境诊断（telos-doctor）

## 这是什么

用户在 Telos 里遇到问题（pi 连不上、会话起不来、卡顿、闪退、模型列表为空等）时，
本技能引导用户走一条**标准排障路径**，用一份脱敏诊断报告替代反复追问环境细节，
快速定位根因。

## 第一步：让用户生成诊断报告

请用户打开 Telos 的**「问题反馈」入口（侧边栏/帮助菜单 → 意见反馈 → 环境诊断 tab）**，
点击**「生成诊断报告」**，然后：
...
- **环境信息**：Telos 版本、安装形态（安装版/便携版/开发态）、OS 版本、Electron/Node/Chrome 版本、
...
- **不代替 pi 做 Agent 行为**：本技能只管 Telos 应用自身的安装/配置/环境问题，不处理
