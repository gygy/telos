# Telos 内置 Prompt 模板（builtin://）

本目录是从 Telos 历史版本中整理还原的**内置 prompt 模板**。

这些模板曾写死在 Telos 主进程 `src/main/prompts/PromptManager.ts` 的
...
- frontmatter `description` 即模板描述；Telos 提示词页中展示的中英文描述
  由 `src/renderer/src/composerBehavior.ts` 的 `BUILTIN_PROMPT_DESC_CN/EN`
  映射提供（该映射仍保留在代码中）
- 安装后这些模板属于用户自建模板，可在 Telos 提示词页正常编辑/删除
