...
> 触发问题：① Telos 引用 DSH 作为后端具体怎么做的？有没有更好的方法？② 我们和官方 dsh-web 是否
...
| 问题 | 结论 |
|---|---|
| 怎么接的 | 官方 **Typert Remote / Connection RPC** 契约 + Telos 自实现的**载体**（utilityProcess + MessagePort fetch 桥）。传输选型是官方明确背书的形态，不是绕路 |
| 有没有更好的方法 | 传输层没有更好的；**但有三个地方我们现在做得比需要的复杂**：两个自建桥在 0.1.5 之后已冗余、载荷契约缺少漂移门禁、官方客户端 caller 被我们手写重实现 |
| 「只差前端」 | 约 90% 成立。剩下的 10% 不是视觉差异而是**能力结构差异**：官方前端内含一个**客户端插件运行时**（`window.__ModuleLoader__` + `__DSH_BOOT__` + `dsh-client-ui-*` roster），Telos 的前端是静态 React，因此 Client 半区插件、官方 UI 贡献点我们结构性缺失 |
...
| 复用 `~/.dsh` | 配置/凭据/会话/预设**已共享**（默认 `~/.dsh`，与 dsh CLI 同一个家）；**profiles 未共享**（用户用 dsh CLI 装的插件 Telos 看不到）；且主进程存在**重复解析**（settings.yaml / .credentials.yaml）应改为走 host RPC |
...
- **组合是自组的**：`patches = loadOverlayPatches(base) + Telos 自有行 + `$DSH_HOME/cordis.patch.yml``，
...
| host 行集 | base + web-app 23 个额外行 | base + Telos 选定行（**缺 7 个**，见 §7） | **能力差异**（部分特性缺） |
| 会话身份 | host 会话即身份 | catalog 映射（`dshSessionId` ↔ Telos sessionId） | 实现差异（为多后端/多项目服务） |
...
| **E. 起 `dsh web` 子进程，Telos 当浏览器客户端** | 多一个端口/进程，需要 `dsh` CLI 在用户机器上，退化回「依赖用户自装」 | **否决**（深融合路线的初衷即此） |
...
4. **19 个转发事件白名单**（`API_REMOTE_FORWARDED_EVENTS`）：含 `approval/request` 与 `user-questions/request`
   （waterfall）以及 `cordis/*`（插件面板手势）、`settings/document-updated`、`api-session/*` 等 —— Telos 的审批/提问桥走的正是这两条 waterfall。
...
| **S2** | `pideck-plugin-bridge`（`/pideck-plugin/rpc`：install/run/stop/uninstall/staticInventory） | `plugin-inventory` 与 `cordis-host-runner` 是 Telos 自己挂的行 → `pluginInventory/list`、`dynamicCordisRunner/*` 端点已注册 | **删除该桥**，改用官方端点；`mergeStaticPluginViews` 这类展示归并留在渲染层 |
| **S3** | `pideck-slash-bridge`（`agent/pre-step` 拦截 `/` 消息） | `commands/execute` 是官方端点（dsh-web 就是客户端直接调它） | 评估后**用端点替代桥**：Telos 侧判断 `/` 命令 → 调 `commands/execute` → 不再发 prompt。少一个 host 插件，且命令语义与官方一致 |
...
`dsh-web-app` 相对 `dsh-base` 多挂 23 行（`ui-*` 与 web 载体除外）。Telos 已挂其中 13 行；**未挂**：

| 行 id | 包 | 影响的端点 | 用户可见后果 |
|---|---|---|---|
| `message-feedback` | dsh-message-feedback | `messageFeedback/{list,put,delete}` | 助手消息的 👍/👎 反馈（官方有，Telos 无） |
| `workspace-files` | dsh-api-workspace-files | `workspaceFiles/{list,read,readAll,readBytes,stat,changes,readRelated}` | 官方文件树/预览走 host 投影（Telos 自己读盘，能力等价但行为可能漂移） |
| `session-reference` + `file-reference-local` | dsh-session-reference / dsh-file-reference-local | `fileReferences/list`、`sessionReferenceResolver/candidates` | `@` 引用候选由 host 计算（Telos 自建引用，等价实现） |
| `session-turn-outline` | dsh-session-turn-outline | `session/turnOutline`（投影） | 官方轮次大纲（Telos 自建 turn 逻辑） |
| `session-log-download` | dsh-session-log-export | host GET 路由（会话日志 ZIP） | 「导出会话日志」入口（G10 的一个选项） |
| `open-in-app` | dsh-host-open-in-app | — | 官方「在外部应用打开」（Telos 有自己的编辑器/系统打开） |
...
- **会话删除语义**：wire 无 `session.delete`（`workspace/archiveSession` 是官方的归档端点，Telos 已自建归档，
  可考虑换用它统一语义）。
...
3. **P2：补挂 7 个 host 行中真有价值的**（`message-feedback`、`workspace-files`、`session-reference`、
   `session-log-download`），逐行评估是否与 Telos 自建实现重复。
