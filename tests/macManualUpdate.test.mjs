...
const macManualUpdate = compileModule("src/main/update/macManualUpdate.ts", (specifier) => {
	...
	if (specifier === "./releaseRepo") {
		return { RELEASES_URL: "https://github.com/gygy/telos/releases" };
	}
	return null;
});
...
test("parseGitHubReleaseVersion accepts a redirected latest-release tag only", () => {
	assert.equal(
		parseGitHubReleaseVersion("https://github.com/gygy/telos/releases/tag/v0.7.4"),
		"0.7.4",
	);
	assert.equal(
		parseGitHubReleaseVersion("https://github.com/gygy/telos/releases/tag/0.7.4-beta.1"),
		"0.7.4-beta.1",
	);
	assert.equal(parseGitHubReleaseVersion("https://github.com/gygy/telos/releases/latest"), null);
	...
	const check = createMacManualUpdateChecker({
		fetchLatestRelease: async (url) => {
			...
			return {
				ok: true,
				status: 200,
				url: "https://github.com/gygy/telos/releases/tag/v0.7.4",
			};
		},
	});
...
test("manual macOS checker surfaces HTTP and malformed redirect failures", async () => {
	...
	const malformed = createMacManualUpdateChecker({
		fetchLatestRelease: async () => ({
			ok: true,
			status: 200,
			url: "https://github.com/gygy/telos/releases/latest",
		}),
	});
